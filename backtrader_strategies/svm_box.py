    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (self.dataclose[0] > self.dataclose[-1]
                    ...
                    and self.Slope[0] > 60
                    and self.Slope[0] > self.Slope[-1]
                    # and self.RSI < 60
                    and self.RSI[0] > self.RSI[-1]
            ):
                # self.mainside = self.buy(size=1)
                # self.trail_price = self.order.created.price
                # self.lowside = self.sell(size=1, exectype=bt.Order.StopTrail, trailpercent=0.03)

                self.mainside = self.buy(transmit=False)
                self.trail_price = self.mainside.created.price
                self.lowside = self.sell(price=self.trail_price * 0.97, size=self.mainside.size, exectype=bt.Order.Stop,
                                         transmit=False, parent=self.mainside)
                self.highside = self.sell(price=self.trail_price * 1.03, size=self.mainside.size, exectype=bt.Order.Limit,
